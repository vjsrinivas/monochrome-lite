...
export class UIRenderer {
    ...
    async renderPinnedItems() {
        ...
        list.innerHTML = pinnedItems
            .map((item) => {
                ...
                } else if (!item.cover && item.type !== 'artist') {
                    if (item.type === 'folder') {
                        iconHTML = `<div class="pinned-item-cover folder-placeholder">${SVG_FOLDER(20)}</div>`;
                    } else if (item.type === 'user-playlist') {
                        iconHTML = `<div class="pinned-item-cover playlist-placeholder">${SVG_MUSIC(20)}</div>`;
                    } else {
                        iconHTML = `<div class="pinned-item-cover default-placeholder">${SVG_MUSIC(20)}</div>`;
                    }
                } else {
                    const coverUrl =
                        item.type === 'artist'
    ...
    coverArtFallback(img) {
        ...
        if (placeholder) {
            img.outerHTML = img.id ? placeholder.replace('<div ', `<div id="${img.id}" `) : placeholder;
        } else {
    ...
    getCoverHTML(
        ...
        if (!cover || (typeof cover === 'string' && (cover.trim() === '' || cover === 'assets/appicon.png'))) {
            return this.createCoverPlaceholderHTML(size);
        }

        let imageUrl;
        if (type === 'artist') {
            imageUrl = this.api.getArtistPictureUrl(cover, size);
        } else if (type === 'album' && !cover.startsWith('music/') && !cover.startsWith('transcode_')) {
            imageUrl = this.api.getAlbumCoverUrl(cover);
        } else {
            imageUrl = this.api.getCoverUrl(cover, size);
        }
    ...
    createFolderCardHTML(folder) {
        ...
        return this.createBaseCardHTML({
            ...
            imageHTML,
            actionButtonsHTML: `
                <button class="edit-folder-btn" data-action="edit-folder" title="Edit Folder">
                    ${SVG_SQUARE_PEN(20)}
                </button>
                <button class="delete-folder-btn" data-action="delete-folder" title="Delete Folder">
                    ${SVG_BIN(20)}
                </button>
            `,
            extraClasses: 'user-folder',
    ...
    createAlbumCardHTML(album) {
        ...
        return this.createBaseCardHTML({
            type: 'album',
            id: album.id,
            href: album._href || `/album/${encodeURIComponent(album.id)}`,
    ...
    createAlbumListItemHTML(album, index) {
        const explicitBadge = hasExplicitContent(album) ? this.createExplicitBadge() : '';
        const qualityBadge = createQualityBadgeHTML(album);
        const isBlocked = contentBlockingSettings?.shouldHideAlbum(album);
        let artistName = '';
        ...
        const isCurrentAlbum = this.player?.currentTrack?.album?.id === album.id;

        return `
            <div class="track-item no-duration${isBlocked ? ' blocked' : ''}${isCurrentAlbum ? ' playing' : ''}" data-album-id="${album.id}" data-type="album" ${isBlocked ? 'title="Blocked: Album blocked"' : ''}>
                ...
                <div class="track-item-info">
                    <div class="track-item-details">
                        <div class="title">${escapeHtml(album.title)} ${explicitBadge} ${qualityBadge}</div>
    ...
    createArtistListItemHTML(artist, index) {
        const isBlocked = contentBlockingSettings?.shouldHideArtist(artist);

        return `
            <div class="track-item no-duration artist-list-item${isBlocked ? ' blocked' : ''}" data-artist-id="${artist.id}" data-type="artist" ${isBlocked ? 'title="Blocked: Artist blocked"' : ''}>
                <div class="track-item-cover artist-list-item-cover">
                    ${this.getCoverHTML(
                        artist.picture,
                        escapeHtml(artist.name),
                        'track-item-cover',
                ...
                <div class="track-item-info">
                    <div class="track-item-details">
                        <div class="title">${escapeHtml(artist.name)}</div>
                    </div>
                </div>
                <div class="track-item-actions">
                    <button class="track-menu-btn" type="button" title="More options">
    ...
    setupLikedTracksSearch(container, filterType) {
        const searchInput = document.getElementById('library-liked-search');
        if (!searchInput || !container) return;

        filterType = filterType || localStorage.getItem('libraryLikedFilter') || 'track';

        const placeholderMap = { track: 'Search liked tracks...', album: 'Search liked albums...', artist: 'Search liked artists...' };
        searchInput.placeholder = placeholderMap[filterType] || placeholderMap.track;

        this.setupSearchClearButton(searchInput);

        const oldListener = searchInput._likedSearchListener;
        if (oldListener) {
            searchInput.removeEventListener('input', oldListener);
        }
        ...
        const listener = () => {
            const query = searchInput.value.toLowerCase().trim();
            const selector = container.classList.contains('card-grid') ? '.card' : '.track-item';
            container.querySelectorAll(selector).forEach((item) => {
                const data = trackDataStore.get(item);
                if (!data) {
                    item.style.display = '';
                    return;
                }
                ...
                let artist = '';
                if (filterType === 'track') {
                    title = getTrackTitle(data) || '';
                    artist = data.artist?.name || data.artists?.[0]?.name || '';
                } else if (filterType === 'album') {
                    title = data.title || '';
                    artist = (typeof data.artist === 'string' ? data.artist : data.artist?.name || data.artists?.[0]?.name || '') || '';
                } else if (filterType === 'artist') {
                    title = data.name || '';
                    artist = title;
                }
                const matches = !query || title.toLowerCase().includes(query) || artist.toLowerCase().includes(query);
                item.style.display = matches ? '' : 'none';
            });
        };

        searchInput._likedSearchListener = listener;
        searchInput.addEventListener('input', listener);
        listener();
    }

    setupAlbumsSearch(container) {
        const searchInput = document.getElementById('library-albums-search');
        if (!searchInput || !container) return;

        this.setupSearchClearButton(searchInput);

        const oldListener = searchInput._albumsSearchListener;
        if (oldListener) {
            searchInput.removeEventListener('input', oldListener);
        }
        ...
        const listener = () => {
            const query = searchInput.value.toLowerCase().trim();
            const selector = container.classList.contains('card-grid') ? '.card[data-album-id]' : '.track-item';
            container.querySelectorAll(selector).forEach((item) => {
                const album = trackDataStore.get(item);
                if (!album) {
                    item.style.display = '';
                    return;
                }
                const title = (album.title || '').toLowerCase();
                const artist = (typeof album.artist === 'string' ? album.artist : album.artist?.name || album.artists?.[0]?.name || '').toLowerCase();
                const matches = !query || title.includes(query) || artist.includes(query);
                item.style.display = matches ? '' : 'none';
            });
        };

        searchInput._albumsSearchListener = listener;
        searchInput.addEventListener('input', listener);
        listener();
    }

    setupAlbumDetailSearch() {
        const searchInput = document.getElementById('album-detail-search');
        if (!searchInput) return;

        this.setupSearchClearButton(searchInput);

        const oldListener = searchInput._albumDetailSearchListener;
        if (oldListener) {
            searchInput.removeEventListener('input', oldListener);
        }
        ...
        const listener = () => {
            const query = searchInput.value.toLowerCase().trim();
            document.querySelectorAll('#album-detail-tracklist .track-item').forEach((item) => {
                const track = trackDataStore.get(item);
                if (!track) {
                    item.style.display = '';
                    return;
        ...
        searchInput._albumDetailSearchListener = listener;
        searchInput.addEventListener('input', listener);
    }

    setupAlbumsViewButtons(currentView) {
        const listBtn = document.getElementById('library-albums-view-list');
        const gridBtn = document.getElementById('library-albums-view-grid');

        if (listBtn) listBtn.classList.toggle('active', currentView === 'list');
        if (gridBtn) gridBtn.classList.toggle('active', currentView === 'grid');
    }

    setupTracksViewButtons(currentView) {
        const listBtn = document.getElementById('library-liked-tracks-view-list');
        const gridBtn = document.getElementById('library-liked-tracks-view-grid');

        if (listBtn) listBtn.classList.toggle('active', currentView === 'list');
        if (gridBtn) gridBtn.classList.toggle('active', currentView === 'grid');
    }

    setupLikedTracksViewButtons(currentView) {
        const listBtn = document.getElementById('library-liked-view-list');
        const gridBtn = document.getElementById('library-liked-view-grid');

        if (listBtn) listBtn.classList.toggle('active', currentView === 'list');
        if (gridBtn) gridBtn.classList.toggle('active', currentView === 'grid');
    }

    setupArtistsSearch(container) {
        const searchInput = document.getElementById('library-artists-search');
        if (!searchInput || !container) return;

        this.setupSearchClearButton(searchInput);

        const oldListener = searchInput._artistsSearchListener;
        if (oldListener) {
            searchInput.removeEventListener('input', oldListener);
        }
        ...
        const listener = () => {
            const query = searchInput.value.toLowerCase().trim();
            const selector = container.classList.contains('card-grid') ? '.card[data-artist-id]' : '.track-item';
            container.querySelectorAll(selector).forEach((item) => {
                const artist = trackDataStore.get(item);
                if (!artist) {
                    item.style.display = '';
                    return;
                }
        ...
        searchInput._artistsSearchListener = listener;
        searchInput.addEventListener('input', listener);
        listener();
    }

    setupArtistsViewButtons(currentView) {
        const listBtn = document.getElementById('library-artists-view-list');
        const gridBtn = document.getElementById('library-artists-view-grid');
    ...
    async renderListWithTracks(
        ...
        Array.from(tempDiv.children).forEach((element, index) => {
            ...
            if (element && track) {
                ...
                if (showCover && track.type !== 'video') {
                    const coverEl = element.querySelector('.track-item-cover');
                    if (coverEl) {
    ...
    async resolveAndApplyCoverArt(track, coverSelector) {
        if (!track?.title) return;
        const coverEl = document.querySelector(coverSelector);
        if (!coverEl) return;
        const existingSrc = coverEl.tagName === 'VIDEO' ? coverEl.poster : coverEl.src;
        if (existingSrc && !existingSrc.includes('appicon.png') && !existingSrc.includes('folder.png')) return;

        const coverArt = await this.api.getCoverArtUrl(track.title);
        if (coverArt?.url) {
            if (coverEl.tagName === 'DIV') {
                ...
                coverEl.src = coverArt.url;
            } else if (coverEl.tagName === 'VIDEO') {
                coverEl.poster = coverArt.url;
            }
        }
    }
    ...
    async updateFullscreenMetadata(track, nextTrack) {
        ...
        if (isRealVideo) {
            ...
            if (videoCoverUrl) {
                ...
                if (track.type !== 'video') {
                    this.api.getCoverArtUrl(track.title).then((coverArt) => {
                        const el = document.getElementById('fullscreen-cover-image');
                        if (!el) return;
                        if (coverArt?.url) {
                            el.src = coverArt.url;
                        }
    ...
    async showFullscreenCover(track, nextTrack, lyricsManager, activeElement) {
        ...
        if (coverImage && !coverImage._fsCoverErrorSetup) {
            coverImage._fsCoverErrorSetup = true;
            coverImage.onerror = () => {
                coverImage.outerHTML = `<div id="fullscreen-cover-image" class="cover-placeholder" style="width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:var(--secondary);color:var(--muted-foreground);"><svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="opacity:0.7"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg></div>`;
            };
        }
        ...
        if (artistEl) {
            ...
            artistEl.onclick = () => {
                if (this.player.currentTrack && this.player.currentTrack.artist) {
                    this.closeFullscreenCover();
                    navigate(`/artist/${encodeURIComponent(this.player.currentTrack.artist.id)}`);
                }
            };
        }
    ...
    async renderResetPasswordPage() {
        ...
        form.onsubmit = async (e) => {
            ...
            try {
                ...
                // PocketBase removed - password reset unavailable
                successEl.textContent = 'Password reset requires a PocketBase server.';
    ...
    async renderLibraryPage() {
        ...
        likedContainer.innerHTML = this.createSkeletonTracks(8, true);
        ...
        await Promise.allSettled([
            feed(api.getCatalogTracks({ limit: 500 }), async (r) => {
                ...
                const tracksView = localStorage.getItem('libraryLikedTracksView') || 'list';
                if (tracks.length > 0) {
                    if (tracksView === 'grid') {
                        tracksContainer.classList.remove('track-list');
                        tracksContainer.classList.add('card-grid');
                        tracksContainer.innerHTML = tracks.map((t) => this.createTrackCardHTML(t)).join('');
                        for (const card of tracksContainer.querySelectorAll('.card')) {
                            const trackId = card.dataset.trackId;
                            if (trackId) {
                                const track = tracks.find((t) => t.id == trackId);
                                if (track) {
                                    trackDataStore.set(card, track);
                                }
                                await this.updateLikeState(card, 'track', trackId);
                            }
                        }
                    } else {
                        tracksContainer.classList.remove('card-grid');
                    ...
                    const tracksToolbar = document.getElementById('library-liked-tracks-toolbar');
                    if (tracksToolbar) tracksToolbar.style.display = 'flex';
                    const tracksShuffleBtn = document.getElementById('shuffle-liked-tracks-btn');
                    if (tracksShuffleBtn) tracksShuffleBtn.style.display = 'flex';
                    this.setupTracksViewButtons(tracksView);
                } else {
                    tracksContainer.innerHTML = createPlaceholder('No tracks in catalog.');
                }
            }),
            feed(api.getCatalogAlbums({ limit: 500 }), async (r) => {
                const albums = r?.albums || r?.items || r || [];
                const albumsView = localStorage.getItem('libraryAlbumsView') || 'grid';

                if (albumsView === 'list') {
                    albumsContainer.classList.remove('card-grid');
                    albumsContainer.classList.add('track-list');
                    albumsContainer.innerHTML = albums.length
                        ? albums.map((a, i) => this.createAlbumListItemHTML(a, i)).join('')
                        : createPlaceholder('No albums in catalog.');
                } else {
                    albumsContainer.classList.remove('track-list');
                ...
                for (const a of albums) {
                    const selector = `[data-album-id="${a.id}"]`;
                    const el = albumsContainer.querySelector(selector);
                    if (el) {
                ...
                if (albums.length > 0) {
                    const albumsToolbar = document.getElementById('library-albums-toolbar');
                    if (albumsToolbar) albumsToolbar.style.display = 'flex';
                    this.setupAlbumsSearch(albumsContainer);
                    this.setupAlbumsViewButtons(albumsView);
                }
            }),
            feed(api.getCatalogArtists({ limit: 500 }), async (r) => {
                const artists = r?.artists || r?.items || r || [];
                const artistsView = localStorage.getItem('libraryArtistsView') || 'grid';

                if (artistsView === 'list') {
                    artistsContainer.classList.remove('card-grid');
                    artistsContainer.classList.add('track-list');
                    artistsContainer.innerHTML = artists.length
                        ? artists.map((a, i) => this.createArtistListItemHTML(a, i)).join('')
                        : createPlaceholder('No artists in catalog.');
                } else {
                    artistsContainer.classList.remove('track-list');
                ...
                for (const a of artists) {
                    const selector = `[data-artist-id="${a.id}"]`;
                    const el = artistsContainer.querySelector(selector);
                    if (el) {
                        trackDataStore.set(el, a);
                ...
                if (artists.length > 0) {
                    const artistsToolbar = document.getElementById('library-artists-toolbar');
                    if (artistsToolbar) artistsToolbar.style.display = 'flex';
                    this.setupArtistsSearch(artistsContainer);
                    this.setupArtistsViewButtons(artistsView);
                }
            }),
            feed(db.getFavorites('track'), () => this.renderLikedContent()),
        ]);

        // Keep the liked tab live: re-render it when favorites change while on this page
        this._onFavoritesChanged ||= () => {
            if (this.currentPage === 'library') void this.renderLikedContent();
        };
        window.removeEventListener('favorites-changed', this._onFavoritesChanged);
        window.addEventListener('favorites-changed', this._onFavoritesChanged);
        ...
        this.setupLikedFilterDropdown();
    }

    async renderLikedContent(filterType) {
        const likedContainer = document.getElementById('library-liked-container');
        if (!likedContainer) return;

        filterType = filterType || localStorage.getItem('libraryLikedFilter') || 'track';
        localStorage.setItem('libraryLikedFilter', filterType);

        let items = [];
        let placeholderText = '';
        let shuffleBtn = document.getElementById('shuffle-liked-btn');

        if (filterType === 'track') {
            items = await db.getFavorites('track');
            placeholderText = 'No liked tracks yet.';
            if (shuffleBtn) shuffleBtn.style.display = 'flex';
        } else if (filterType === 'album') {
            items = await db.getFavorites('album');
            placeholderText = 'No liked albums yet.';
            if (shuffleBtn) shuffleBtn.style.display = 'none';
        } else if (filterType === 'artist') {
            items = await db.getFavorites('artist');
            placeholderText = 'No liked artists yet.';
            if (shuffleBtn) shuffleBtn.style.display = 'none';
        }
        ...
        if (items && items.length > 0) {
            ...
            if (filterType === 'track') {
                ...
                if (likedLayout === 'grid') {
                    ...
                    likedContainer.innerHTML = items.map((t) => this.createTrackCardHTML(t)).join('');
                    for (const card of likedContainer.querySelectorAll('.card')) {
                        const trackId = card.dataset.trackId;
                        if (trackId) {
                            const track = items.find((t) => t.id == trackId);
                    ...
                    likedContainer.classList.remove('card-grid');
                    likedContainer.classList.add('track-list');
                    await this.renderListWithTracks(likedContainer, items, true);
                }
                this.setupLikedTracksViewButtons(likedLayout);
            } else if (filterType === 'album') {
                const likedLayout = localStorage.getItem('libraryLikedView') || 'grid';
                if (likedLayout === 'grid') {
                    likedContainer.classList.remove('track-list');
                    likedContainer.classList.add('card-grid');
                    likedContainer.innerHTML = items.map((a) => this.createAlbumCardHTML(a)).join('');
                    for (const card of likedContainer.querySelectorAll('.card')) {
                        const albumId = card.dataset.albumId;
                        if (albumId) {
                            const album = items.find((a) => a.id == albumId);
                            if (album) trackDataStore.set(card, album);
                            await this.updateLikeState(card, 'album', albumId);
                        }
                    }
                } else {
                    likedContainer.classList.remove('card-grid');
                    likedContainer.classList.add('track-list');
                    likedContainer.innerHTML = items.length
                        ? items.map((a, i) => this.createAlbumListItemHTML(a, i)).join('')
                        : createPlaceholder('No liked albums yet.');
                }
                this.setupLikedTracksViewButtons(likedLayout);
            } else if (filterType === 'artist') {
                const likedLayout = localStorage.getItem('libraryLikedView') || 'grid';
                if (likedLayout === 'grid') {
                    likedContainer.classList.remove('track-list');
                    likedContainer.classList.add('card-grid');
                    likedContainer.innerHTML = items.map((a) => this.createArtistCardHTML(a)).join('');
                    for (const card of likedContainer.querySelectorAll('.card')) {
                        const artistId = card.dataset.artistId;
                        if (artistId) {
                            const artist = items.find((a) => a.id == artistId);
                            if (artist) trackDataStore.set(card, artist);
                            await this.updateLikeState(card, 'artist', artistId);
                        }
                    }
                } else {
                    likedContainer.classList.remove('card-grid');
                    likedContainer.classList.add('track-list');
                    likedContainer.innerHTML = items.length
                        ? items.map((a, i) => this.createArtistListItemHTML(a, i)).join('')
                        : createPlaceholder('No liked artists yet.');
                }
                this.setupLikedTracksViewButtons(likedLayout);
            }

            this.setupLikedTracksSearch(likedContainer, filterType);
        } else {
            likedContainer.classList.remove('card-grid');
            likedContainer.classList.add('track-list');
            likedContainer.innerHTML = createPlaceholder(placeholderText);
    ...
    updateFilterLabel(filterType) {
        const labelMap = { track: 'Tracks', album: 'Albums', artist: 'Artists' };
        const label = document.getElementById('library-liked-filter-label');
        if (label) label.textContent = labelMap[filterType] || 'Tracks';

        document.querySelectorAll('.library-liked-filter-option').forEach((btn) => {
            btn.classList.toggle('active', btn.dataset.filter === filterType);
        });
    }

    setupLikedFilterDropdown() {
        if (this._likedFilterSetup) return;
        this._likedFilterSetup = true;

        const filterBtn = document.getElementById('library-liked-filter-btn');
        const filterMenu = document.getElementById('library-liked-filter-menu');
        if (!filterBtn || !filterMenu) return;

        const savedFilter = localStorage.getItem('libraryLikedFilter') || 'track';
        this.updateFilterLabel(savedFilter);
        ...
        filterMenu.querySelectorAll('.library-liked-filter-option').forEach((option) => {
            option.addEventListener('click', () => {
                const filterType = option.dataset.filter;
                filterMenu.classList.remove('active');
                this.renderLikedContent(filterType);
            });
        });
    }
    ...
    async renderHomeRecent() {
        ...
        if (recentContainer) {
            ...
            try {
                if (recents.albums?.length) items.push(...recents.albums.slice(0, 4).map((i) => ({ ...i, _kind: 'album' })));
                if (recents.playlists?.length)
                    items.push(...recents.playlists.slice(0, 4).map((i) => ({ ...i, _kind: 'playlist' })));
                if (recents.mixes?.length) items.push(...recents.mixes.slice(0, 4).map((i) => ({ ...i, _kind: 'mix' })));
            } catch (e) {
                console.error('[renderHomeRecent] Failed to read recents:', e);
                recentContainer.innerHTML = createPlaceholder('Failed to load recent items.');
                return;
            }
            ...
            if (displayItems.length > 0) {
                try {
                    ...
                    console.error('[renderHomeRecent] Failed to render cards:', e);
                    recentContainer.innerHTML = createPlaceholder('Failed to render recent items.');
                    return;
                }
                ...
                for (const item of displayItems) {
                    ...
                    if (el) {
                        ...
                        try {
                            ...
                            console.error('[renderHomeRecent] Failed to update like state:', e);
                        }
                    }
                }
            } else {
    ...
    async renderSearchPage(query) {
        ...
        const foldersContainer = document.getElementById('search-folders-container');
        const podcastsContainer = document.getElementById('search-podcasts-container');

        tracksContainer.innerHTML = '<div class="placeholder">Searching...</div>';
        ...
        try {
            ...
            // Local folders + playlists from IndexedDB
            const [localFolders, localPlaylists] = await Promise.all([db.getFolders(), db.getPlaylists()]);
            const q = query.toLowerCase();
            const matchedFolders = localFolders.filter((f) => f.name.toLowerCase().includes(q));
            const matchedPlaylists = localPlaylists.filter((p) => p.name.toLowerCase().includes(q));

            foldersContainer.innerHTML = matchedFolders.length
                ? matchedFolders.map((f) => this.createFolderCardHTML(f)).join('')
                : createPlaceholder('No folders found.');

            for (const folder of matchedFolders) {
                const el = foldersContainer.querySelector(`[data-folder-id="${folder.id}"]`);
                if (el) trackDataStore.set(el, folder);
            }

            if (matchedPlaylists.length > 0 || finalPlaylists.length === 0) {
                if (matchedPlaylists.length > 0) {
                    const localHTML = matchedPlaylists.map((p) => this.createUserPlaylistCardHTML(p)).join('');
                    if (finalPlaylists.length === 0) {
                        playlistsContainer.innerHTML = localHTML;
                    } else {
                        playlistsContainer.insertAdjacentHTML('beforeend', localHTML);
                    }
            ...
            for (const playlist of matchedPlaylists) {
                const el = playlistsContainer.querySelector(`[data-user-playlist-id="${playlist.id}"]`);
            ...
            foldersContainer.innerHTML = errorMsg;
    ...
    async renderAlbumPage(albumId, provider = null) {
        console.log('[renderAlbumPage] called with albumId:', albumId);
        ...
        try {
            ...
            console.log('[renderAlbumPage] API returned album:', album?.title, 'tracks:', tracks?.length);
            ...
            let coverUrl = videoCoverUrl || (album.cover ? this.api.getCoverUrl(album.cover) : album.title ? this.api.getAlbumCoverUrl(album.title) : null);
            ...
            if (backgroundSettings.isEnabled() && coverUrl) {
                await this.extractAndApplyColor(coverUrl);
            }
            ...
            // Setup search for album detail page
            this.setupAlbumDetailSearch();

            try { recentActivityManager.addAlbum(album); } catch (e) { console.warn('[recentActivityManager] addAlbum failed:', e); }
    ...
    async renderPlaylistPage(playlistId, source = null, _provider = null) {
        console.log('[renderPlaylistPage] called with playlistId:', playlistId, 'source:', source);
        ...
        try {
            console.log('[renderPlaylistPage] trying to load playlistId:', playlistId, 'source:', source);
            ...
            if (source === 'user' || (!source && isUUID)) {
                ...
                console.log('[renderPlaylistPage] loaded from DB:', playlistData?.name);
            }
            ...
            if (playlistData) {
                ...
                try { recentActivityManager.addPlaylist({
                    ...
                    isUserPlaylist: true,
                }); } catch (e) { console.warn('[recentActivityManager] addPlaylist(user) failed:', e); }
                ...
                // Render API playlist
                console.log('[renderPlaylistPage] fetching from API');
                let apiResult = await this.api.getPlaylist(playlistId);
                console.log('[renderPlaylistPage] API returned playlist:', apiResult?.playlist?.name);
                ...
                try { recentActivityManager.addPlaylist(playlist); } catch (e) { console.warn('[recentActivityManager] addPlaylist(mix) failed:', e); }
        ...
        try {
            ...
            if (folder.playlists?.length > 0) {
                ...
                if (playlists.length > 0) {
                    ...
                    playlists.forEach((p) => {
                        const el = container.querySelector(`[data-user-playlist-id="${p.id}"]`);
                        if (el) {
                            trackDataStore.set(el, p);
                            const removeBtn = document.createElement('button');
                            removeBtn.className = 'remove-from-folder-btn';
                            removeBtn.title = 'Remove from folder';
                            removeBtn.innerHTML = SVG_BIN(16);
                            removeBtn.onclick = async (e) => {
                                e.stopPropagation();
                                const updated = await db.removePlaylistFromFolder(folderId, p.id);
                                el.remove();
                                if (updated.playlists.length === 0) {
                            ...
                            if (imageWrapper) imageWrapper.appendChild(removeBtn);
                        }
                    });
                } else {
    ...
    async renderMixPage(mixId, provider = null) {
        console.log('[renderMixPage] called with mixId:', mixId);
        ...
        try {
            ...
            console.log('[renderMixPage] API returned mix:', mix?.title, 'tracks:', tracks?.length);
...
if (imageEl.tagName === 'VIDEO') {
            ...
            try { recentActivityManager.addMix(mix); } catch (e) { console.warn('[recentActivityManager] addMix failed:', e); }
    ...
    async renderArtistPage(artistId, provider = null) {
        console.log('[renderArtistPage] called with artistId:', artistId);
        ...
        try {
            ...
            console.log('[renderArtistPage] API returned artist:', artist?.name);
            ...
            imageEl.onerror = () => this.coverArtFallback(imageEl);
            ...
            try { recentActivityManager.addArtist(artist); } catch (e) { console.warn('[recentActivityManager] addArtist failed:', e); }
    ...
    async renderTrackPage(trackId, provider = null) {
        ...
        try {
            ...
            // Try gateway cover art first, fall back to track.image/cover/album.cover
            let coverUrl = videoCoverUrl || this.api.getCoverUrl(track.image || track.cover || track.album?.cover);
            if (!videoCoverUrl && track.title) {
                const gatewayCoverArt = await this.api.getCoverArtUrl(track.title);
                if (gatewayCoverArt?.url) {
                    coverUrl = gatewayCoverArt.url;
                }
            }
    ...
    async renderPodcastsBrowsePage() {
        ...
        try {
            const podcastsAPI = { getRecentPodcasts: async () => ({ items: [] }) };
            const recentResult = await podcastsAPI.getRecentPodcasts({ max: 24 });
            if (recentResult.items.length > 0) {
                recentContainer.innerHTML = recentResult.items
                    .map((podcast) => this.createPodcastCardHTML(podcast))
                    .join('');
                this.attachPodcastCardListeners(recentContainer, recentResult.items);
            } else {
                recentContainer.innerHTML = createPlaceholder('No recent podcasts found.');
            }
        } catch (error) {
            console.error('Failed to load recent podcasts:', error);
            recentContainer.innerHTML = createPlaceholder('Failed to load recent podcasts.');
        }
    ...
    async renderPodcastPage(podcastId) {
        ...
        try {
            const podcastsAPI = { getPodcastById: async () => null };
    ...
    async loadAllPodcastEpisodes() {
        ...
        try {
            const podcastsAPI = { getPodcastEpisodes: async () => ({ items: [] }) };
    ...
    async renderPodcastSearchResults(query) {
        ...
        try {
            const podcastsAPI = { searchPodcasts: async () => ({ items: [] }) };
    ...
    updateCardPlayButtons() {
        ...
        const isPlaying = !player.activeElement.paused;

        // Update track cards
        document.querySelectorAll('.card[data-track-id], .track-item[data-track-id]').forEach((card) => {
            const trackId = card.dataset.trackId;
            const playBtn = card.querySelector('.card-play-btn');
            if (playBtn && trackId === currentTrackId) {
                playBtn.innerHTML = isPlaying ? SVG_PAUSE(20) : SVG_PLAY(20);
        ...
        document.querySelectorAll('.card[data-album-id]').forEach((card) => {
            const albumId = card.dataset.albumId;
            const playBtn = card.querySelector('.card-play-btn');
            const currentAlbumId = player.currentTrack?.album?.id;
            if (playBtn && currentAlbumId && albumId === currentAlbumId) {
                playBtn.innerHTML = isPlaying ? SVG_PAUSE(20) : SVG_PLAY(20);
                playBtn.title = isPlaying ? 'Pause' : 'Play';
            }
        });
        ...
        document.querySelectorAll('.card[data-playlist-id]').forEach((card) => {
            ...
            const playBtn = card.querySelector('.card-play-btn');
            const currentPlaylistId = player.currentTrack?.playlist?.uuid;
            if (playBtn && currentPlaylistId && playlistId === currentPlaylistId) {
                playBtn.innerHTML = isPlaying ? SVG_PAUSE(20) : SVG_PLAY(20);
                playBtn.title = isPlaying ? 'Pause' : 'Play';
            }
        });

        // Update user-playlist cards
        document.querySelectorAll('.card[data-user-playlist-id]').forEach((card) => {
            const playlistId = card.dataset.userPlaylistId;
            const playBtn = card.querySelector('.card-play-btn');
            const currentPlaylistId = player.currentTrack?.playlist?.uuid || player.currentTrack?.playlist?.id;
            if (playBtn && currentPlaylistId && playlistId === currentPlaylistId) {
                playBtn.innerHTML = isPlaying ? SVG_PAUSE(20) : SVG_PLAY(20);
                playBtn.title = isPlaying ? 'Pause' : 'Play';
            }
